...
@section('script')
    <script>
        function downloadPDF2() {
            ...
            const opt = {
                margin: [0.4, 0.2],
                filename: 'notulen_qcc_l5.pdf', // Nama file PDF yang diunduh
                image: {
                    type: 'jpeg',
                    quality: 1
                },
        ...
        $(document).ready(function() {
            ...
            $('.approve-btn').click(function() {
                ...
                if (confirm('Anda yakin ingin approve?')) {
                    ...
                    $.ajax({
                        url: '{{ route('approveQcc5', ['id' => $circle->id]) }}',
                        method: 'POST',
                        headers: {
                ...
                <div class="card">
                    ...
                    <div class="card-body p-1 p-md-3">
                        ...
                        <div class="d-flex justify-content-between border border-dark border-start-0 border-end-0 mb-2 p-2">
                            ...
                            <h6 class="text-end text-dark my-auto">LANGKAH 5 - IMPLEMENTASI PERBAIKAN</h6>
                        </div>
                        <div class="mb-2 ">
                        ...
                        <div class="mb-2 ">
                            ...
                            <table class="table table-borderless">
                                ...
                                @foreach ($implemens as $implemen)
                                    <tr class="table-success">
                                        <td>{{ $implemen->what }}</td>
                                        <td>{{ $implemen->how ?: '-' }}</td>
                                        <td>{{ $implemen->why ?: '-' }}</td>
                                        <td>{{ $implemen->where ?: '-' }}</td>
                                        <td>{{ $implemen->when ?: '-' }}</td>
                                        <td>{{ $implemen->how_much ?: '-' }}</td>
                                    </tr>
                                @endforeach
                            </table>
                        </div>
                        <div class="mb-2 ">
                            <label for="nama">Design Improvement</label>
                            <img src="{{ asset($fotoImprove) }}" alt="" width="350px" class="d-block rounded-1">
                        </div>
                    </div>
                    @if ($comment)
                        <div class="mb-2 ">
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <form method="POST" action="{{ route('approveCommentQcc5') }}"
