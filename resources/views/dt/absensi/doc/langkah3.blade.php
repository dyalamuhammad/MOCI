...
@section('script')
    ...
    <script>
        function downloadPDF2() {
            ...
            const opt = {
                margin: 0,
                filename: 'notulen_dt_l3.pdf', // Nama file PDF yang diunduh
                image: {
                    type: 'jpeg',
                    quality: 1
                },
        ...
        $(document).ready(function() {
            ...
            $('.approve-btn').click(function() {
                ...
                if (confirm('Anda yakin ingin approve?')) {
                    ...
                    $.ajax({
                        url: '{{ route('approveDt3', ['id' => $circle->id]) }}',
                        method: 'POST',
                        headers: {
                ...
                <div class="card">
                    ...
                    <div class="card-body p-1 p-md-3">
                        <div class="card-body p-1 p-md-3">
                            ...
                            <div class="mb-2">
                                <label for="nama">Customer Journey</label>
                                <table class="table table-borderless">
                                    <tr>
                                        <th class="bg-green text-light">Stage</th>
                                        <th class="bg-green text-light">Touch Point</th>
                                        <th class="bg-green text-light">Doing</th>
                                        <th class="bg-green text-light">Expect</th>
                                        <th class="bg-green text-light">Thinking</th>
                                        <th class="bg-green text-light">Pain Point</th>
                                    </tr>
                                    @foreach ($notulen3dt as $item)
                                        <tr class="table-success">
                                            <td>{{ $item->stage }}</td>
                                            <td>{{ $item->touch_point }}</td>
                                            <td>{{ $item->doing }}</td>
                                            <td>{{ $item->expect }}</td>
                                            <td>{{ $item->thinking }}</td>
                                            <td>{{ $item->pain_point }}</td>
                                        </tr>
                                    @endforeach
                                </table>
                            </div>
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <form method="POST" action="{{ route('approveCommentDt3') }}"
